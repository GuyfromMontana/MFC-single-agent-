...
- [ ] **`postgrest_client_timeout` is the library default of 120s** — a hung Supabase connection can hold a webhook far past Retell's patience. `sb_exec` bounds retries, not the per-attempt wait. Set an explicit `ClientOptions(postgrest_client_timeout=...)` (~10s) after deciding the right ceiling.
